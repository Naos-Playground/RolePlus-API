// -----------------------------------------------------------------------
// <copyright file="LayerIIDecoderBase.cs" company="NaoUnderscore">
...
#pragma warning disable SA1401 // Fields should be private
    ...
    internal abstract class LayerIIDecoderBase : LayerDecoderBase
    {
        ...
        protected LayerIIDecoderBase(int[][] allocLookupTable, int granuleCount)
        {
            ...
            _allocation = new[] { new int[SBLIMIT], new int[SBLIMIT] };
            _scfsi = new[] { new int[SBLIMIT], new int[SBLIMIT] };
            _samples = new[] { new int[SBLIMIT * SSLIMIT * GranuleCount], new int[SBLIMIT * SSLIMIT * GranuleCount] };

            // NB: ReadScaleFactors(...) requires all three granules, even in Layer I
            _scalefac = new[] { new int[3][], new int[3][] };
            for (int i = 0; i < 3; i++)
            {
        ...
        {
            int subBandCount = rateTable.Length;
            if (_jsbound > subBandCount)
                _jsbound = subBandCount;
        ...
        private void ReadSamples(IMpegFrame frame)
        {
            ...
            for (int ss = 0, idx = 0; ss < SSLIMIT; ss++, idx += SBLIMIT * (GranuleCount - 1))
            {
                    ...
                    for (int ch = 0; ch < _channels; ch++)
                    {
                        if (ch == 0 || sb < _jsbound)
                        {
                            ...
                            if (alloc != 0)
                            {
                                ...
                                {
                                    // non-grouping
                                    for (int gr = 0; gr < GranuleCount; gr++)
