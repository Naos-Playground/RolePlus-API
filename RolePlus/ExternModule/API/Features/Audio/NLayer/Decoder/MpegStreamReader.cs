// -----------------------------------------------------------------------
// <copyright file="MpegStreamReader.cs" company="NaoUnderscore">
            ...
            lock (_frameLock)
            {
                ...
                try
                {
                    if (Read(_readOffset, syncBuf, 0, 4) == 4)
                    {
                        ...
                        do
                        {
                            uint sync = (uint)((syncBuf[0] << 24) | (syncBuf[1] << 16) | (syncBuf[2] << 8) |
                                                syncBuf[3]);
