// -----------------------------------------------------------------------
// <copyright file="ShotPelletPatch.cs" company="NaoUnderscore">
// Copyright (c) NaoUnderscore. All rights reserved.
// Licensed under the CC BY-SA 3.0 license.
// </copyright>
// -----------------------------------------------------------------------

namespace RolePlus.ExternModule.API.Engine.Framework.Patches
{
    using HarmonyLib;
        ...
        private static bool Prefix(BuckshotHitreg __instance, Vector2 pelletSettings, Ray originalRay, Vector2 offsetVector)
        {
            ...
            if (Physics.Raycast(ray, out RaycastHit hit, __instance.Firearm.BaseStats.MaxDistance(), StandardHitregBase.HitregMask))
            {
                ...
                if (destructible != null)
                {
                    ...
                    if (__instance.CanShoot(destructible) && destructible.Damage(damage, new PlayerStatsSystem.FirearmDamageHandler(__instance.Firearm, damage, false), hit.point))
                    {
